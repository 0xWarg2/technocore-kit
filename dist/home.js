...
import { IdentityError } from "./protocol.js";
...
export function technocoreHome(env = process.env) {
...
export function defaultIdentityPath(env = process.env) {
    return join(technocoreHome(env), "identity.pem");
}
/** Default passphrase file path inside the home. */
export function defaultPassphrasePath(env = process.env) {
    return join(technocoreHome(env), "passphrase");
}
function readPassphraseFile(path, label) {
    let contents;
    try {
        contents = readFileSync(path, "utf8");
    }
    catch (cause) {
        throw new IdentityError(`cannot read ${label} ${resolve(path)}: ${cause.message}`);
    }
...
function requirePrivateMode(path) {
    // POSIX mode bits carry no meaning on Windows, where Node synthesises them.
    if (process.platform === "win32")
        return;
    const mode = statSync(path).mode & 0o777;
    if ((mode & 0o077) !== 0) {
        throw new IdentityError(`${path} is readable by other users (mode ${mode.toString(8)}); ` +
            `run: chmod 600 ${path}`);
    }
}
/**
 ...
 */
export function findPassphrase(env = process.env) {
 ...
 */
export function resolveIdentityPath(explicit, env = process.env) {
    if (explicit !== undefined && explicit !== "")
        return explicit;
    const configured = env["TECHNOCORE_IDENTITY"];
    if (configured !== undefined && configured !== "")
        return configured;
    if (existsSync("identity.pem"))
        return "identity.pem";
 ...
 */
export function setupIdentity(env = process.env) {
    ...
    if (existsSync(identityPath)) {
        ...
        if (passphrase === undefined) {
            throw new IdentityError(`${identityPath} already exists but its passphrase was not found; ` +
    ...
    try {
        ...
        if (error.code !== "EEXIST") {
            throw new IdentityError(`cannot write ${passphrasePath}: ${String(error)}`);
        }
