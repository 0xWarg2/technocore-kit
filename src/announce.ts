...
import { verifyContributionProof } from "./proof.ts";
import {
  MULTIBASE_LENGTH,
  normalizeMessage,
  ProtocolError,
...
function requireDid(did: unknown): string {
...
function requireArtifactUrl(artifactUrl: unknown): string {
  if (typeof artifactUrl !== "string" || artifactUrl !== artifactUrl.trim()) {
    throw new ProtocolError(
      "artifact URL must be a string without surrounding whitespace",
    );
  }
  let parsed: URL;
  try {
...
function requireSeq(seq: unknown): number {
 ...
 */
export function announcedProof(
  proof: Record<string, unknown>,
  did: string,
): AnnouncedProof {
  verifyContributionProof(proof);
  const proofDid = proof["did"] as string;
  if (proofDid !== requireDid(did)) {
    throw new ProtocolError(
      `proof is signed by ${proofDid}, not by this agent (${did})`,
    );
  }
  return {
    did: proofDid,
    artifactUrl: proof["artifact_url"] as string,
    commit: proof["commit"] as string,
  };
}
 ...
 * this kit produced.
 */
export function roomAnnouncement(announcement: RoomAnnouncement): string {
...
export function postAnnouncement(announcement: PostAnnouncement): string {
  ...
  if (proof !== undefined) {
    if (
      announcement.artifactUrl !== undefined &&
      announcement.artifactUrl !== proof.artifactUrl
    ) {
      throw new ProtocolError(
        `artifact URL ${announcement.artifactUrl} disagrees with the proof, ` +
  ...
  if (artifactUrl !== undefined) {
    ...
    if (proof !== undefined) lines.push(`Commit: ${proof.commit}`);
  }
  return `${lines.join("\n")}\n`;
}
