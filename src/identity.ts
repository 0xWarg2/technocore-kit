...
export function didFromRawPublicKey(rawPublicKey: Uint8Array): string {
  const prefixed = new Uint8Array(
    MULTICODEC_ED25519.length + rawPublicKey.length,
  );
...
export function didFromPrivateKey(privateKey: KeyObject): string {
...
export function signBytes(privateKey: KeyObject, payload: Uint8Array): string {
  const encoded = Buffer.from(cryptoSign(null, payload, privateKey)).toString(
    "base64url",
  );
...
export function verifyBytes(
  ...
  if (typeof signature !== "string" || !SIGNATURE_PATTERN.test(signature)) {
    throw new ProtocolError(
      "signature must contain 86 unpadded base64url characters",
    );
  }
 ...
 */
export function createIdentityFile(path: string, passphrase: string): string {
  const resolved = resolve(path);
  if (typeof passphrase !== "string" || passphrase.length < MIN_PASSPHRASE_CHARS) {
    throw new IdentityError(
      `identity passphrase must contain at least ${MIN_PASSPHRASE_CHARS} characters`,
    );
  }
  ...
  try {
    ...
    if (code === "EEXIST") {
      throw new IdentityError(
        `refusing to overwrite existing identity: ${resolved}`,
      );
    }
    throw new IdentityError(
      `cannot write encrypted identity ${resolved}: ${String(error)}`,
    );
  }
...
export function loadIdentity(path: string, passphrase: string): KeyObject {
  const resolved = resolve(path);
  let pemBytes: Buffer;
  try {
