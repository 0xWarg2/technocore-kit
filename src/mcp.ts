...
import { didFromPrivateKey, loadIdentity } from "./identity.ts";
import {
...
function identity(): KeyObject {
  ...
  if (!passphrase) {
    throw new Error(
      `no identity is set up yet at ${keyPath}; run the technocore_setup ` +
...
serveStdio(() => {
  ...
  server.registerTool(
    ...
    {
      description:
        "Create this agent's Technocore identity if it does not exist yet: an " +
