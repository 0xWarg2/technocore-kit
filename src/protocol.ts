...
const BASE58BTC_INDEX = new Map<string, bigint>(
  [...BASE58BTC_ALPHABET].map((character, index) => [character, BigInt(index)]),
);
...
export function base58btcEncode(data: Uint8Array): string {
  let zeroes = 0;
  while (zeroes < data.length && data[zeroes] === 0) zeroes += 1;
  let number = 0n;
  ...
  for (const character of value) {
    ...
    if (digit === undefined) {
      throw new ProtocolError(
        `invalid base58btc character: ${JSON.stringify(character)}`,
      );
    }
...
export function normalizeMessage(text: string): string {
  ...
  if (length > MAX_MESSAGE_CHARS) {
    throw new ProtocolError(
      `message has ${length} characters; maximum is ${MAX_MESSAGE_CHARS}`,
    );
  }
  return normalized;
}
...
export function terminalSafeDetail(value: unknown): string {
...
export function nextNonce(): string {
  return validateNonce(BigInt(Date.now()) * 1_000_000n);
}
...
export function validateBaseUrl(baseUrl: string): string {
  ...
  if (!parsed.host || parsed.search || parsed.hash) {
    throw new ProtocolError(
      "base URL must contain a host and no query or fragment",
    );
  }
